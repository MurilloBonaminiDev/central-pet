        ...
        {data.map((item, index) => {
          ...
          return (
            <div key={item.label} className="flex flex-1 flex-col items-center gap-2">
              <span className="text-xs font-semibold text-[var(--color-fg-muted)]">{item.value}</span>
              ...
              <span className="text-xs font-medium text-[var(--color-fg-subtle)]">{item.label}</span>
            </div>
          );
        })}
      </div>
