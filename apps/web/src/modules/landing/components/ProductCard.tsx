...
import { formatProductPrice } from '@modules/products';
...
export function ProductCard({ product }: ProductCardProps) {
  const whatsappUrl = `https://wa.me/${CLINIC.phone}?text=${encodeURIComponent(
    `Olá! Quero comprar o produto "${product.name}" (${formatProductPrice(product.price_cents)}) na Central Pet.`,
  )}`;

  return (
    <article className="product-card flex h-full flex-col overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[var(--color-bg-elevated)] shadow-[var(--shadow-sm)]">
      <div className="relative aspect-[4/3] overflow-hidden bg-[var(--color-bg-muted)]">
        <img
          ...
          loading="lazy"
        />
        <span className="absolute left-3 top-3 rounded-full bg-[var(--color-bg-elevated)]/95 px-2.5 py-1 text-xs font-semibold text-[var(--color-brand-700)] shadow-[var(--shadow-sm)]">
          {product.category}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-4 font-display text-xl font-semibold text-[var(--color-brand-700)]">
        ...
        <a
          ...
          className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] bg-[#25D366] px-4 text-sm font-semibold text-white transition-transform hover:scale-[1.02]"
        >
          <IconWhatsApp />
